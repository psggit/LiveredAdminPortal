...
class SpecialRestriction extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      zoneRestrictions: props.data.city_special_days.concat(props.data.state_special_days)
    }
  ...
  createCitySpecialDay() {
    ...
    Api.createCitySpecialDay({
      ...
      city_id: data.state.selectedCityIdx,
                    ...
                    zoneRestrictions.map((item, i) => {
                      return (
                        <tr onClick={() => this.deleteSpecialDay(`${item.city_id !== undefined ? `city_${item.id}` : `state_${item.id}`}`)}>
                          ...
                          <td>{(item.from_time).substring(11, 16)}</td>
                          <td>{(item.to_time).substring(11, 16)}</td>
